<template lang="html">
	<div class="listModule">
		<div class="list_header">
			<ym-hbt :to='{name:"saleHome"}' title='销售列表'></ym-hbt>
		</div>
		<div class="list_setbar">
			<div class="list_setbar_options">
				...
				<div class="search_input">
					...
					<input class="search_input_placeholder" type="text" v-model='option.orderNo' @change='screenBar' placeholder="销售单号">
				</div>
			</div>
        ...
        <div class="selection" :class="{now:option.stat===2}" @click='option.stat=2;screen()'>30天</div>
				...
				<div class="selection" v-if='selections[1].show' :class="{now:option[selections[1].key]}">
					<el-select v-model="option[selections[1].key]" :placeholder="selections[1].name" @change='screen' clearable>
						<el-option v-if='v&&v!==""' v-for='(v,k) in options[selections[1].key]' :label="contactIdList[v]" :value="v" :key='k'></el-option>
					</el-select>
				</div>
				<div class="selection" v-if='selections[2].show' :class="{now:option[selections[2].key]||option[selections[2].key]===0}">
					<el-select v-model="option[selections[2].key]" :placeholder="selections[2].name" @change='screen' clearable>
						<el-option v-if='v&&v!==""||v===0' v-for='(v,k) in options[selections[2].key]' :label="isTaxList[v]" :value="v" :key='k'></el-option>
					</el-select>
				</div>
				<div class="selection" v-if='selections[3].show' :class="{now:option[selections[3].key]||option[selections[3].key]===0}">
					<el-select v-model="option[selections[3].key]" :placeholder="selections[3].name" @change='screen' clearable>
						<el-option v-if='v&&v!==""||v===0' v-for='(v,k) in options[selections[3].key]' :label="openApproveStateList[v]" :value="v" :key='k'></el-option>
					</el-select>
				</div>
				<div class="selection" v-if='selections[4].show' :class="{now:option[selections[4].key]||option[selections[4].key]===0}">
					<el-select v-model="option[selections[4].key]" :placeholder="selections[4].name" @change='screen' clearable>
						<el-option v-if='v&&v!==""||v===0' v-for='(v,k) in options[selections[4].key]' :label="v" :value="v" :key='k'></el-option>
					</el-select>
				</div>
		...
		<div class="list_main" :class="{se:isSelect}">
					...
					<el-table :data='dataList' height='100%' @selection-change="handleSelectionChange" v-loadMethod='load' modname='.el-table__body-wrapper' loadmore='loadmore'>
						...
						<el-table-column label='销售单号' show-overflow-tooltip>
							<template slot-scope='scope'>
								<router-link :to="{ name: 'saleDeta', query: {id:scope.row.id,backName:'saleList'} }">{{scope.row.orderNo}}</router-link>
							</template>
						</el-table-column>
						...
						<el-table-column v-if='tableShow.orderAmount' label='金额' prop='orderAmount' show-overflow-tooltip></el-table-column>
            ...
            <el-table-column v-if='tableShow.checkApproveState' align="center" label='收款' width='50'>
              <template slot-scope='scope'>
                <i class="iconfont weijinhang icon_noFinish" v-if="scope.row.receivableAmount!=0"></i>
                <i class="iconfont yiwancheng1 icon_finish" v-if="scope.row.receivableAmount==0"></i>
              </template>
            </el-table-column>
					...
					<ul class="card_ul">
						<li class="card_li" v-for='(val,key) in dataList' :key='key'>
							<div class="card_li_con">
								<div class="content" @click="$router.push({name:'saleDeta',query:{id:val.id,backName:'saleList'}})">
	...
	export default {
	    ...
	    tableShow:function() {
	      ...
	      return temp;
			...
			selections:function(){
				return JSON.parse(this.$store.state.setJson).selection.SL;
			},
			tableOptions:function(){
				return JSON.parse(this.$store.state.setJson).tableOptions.SL;
			},
	  },
		methods: {
			/**
	    ...
	    **/
	    exportExcel(){
	      ...
	      require.ensure([],()=>{
	          ...
	          export_json_to_excel(tHeader,data,'销售单列表');
	  　　})
	    },
		},
		mounted() {
