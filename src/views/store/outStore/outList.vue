<template lang="html">
	<div class="listModule">
		<div class="list_header">
			<ym-hbt :to='{name:"outStoreHome"}' title='出库列表'></ym-hbt>
		</div>
					...
					<el-table :data='dataList' height='100%' @selection-change="handleSelectionChange" v-loadMethod='load' modname='.el-table__body-wrapper' loadmore='loadmore'>
						...
						<el-table-column label='出库单号' show-overflow-tooltip>
							<template slot-scope='scope'>
								<router-link :to="{ name:'outDeta',query:{id:scope.row.id}}">{{scope.row.id}}</router-link>
							</template>
						</el-table-column>
						<el-table-column v-if="tableShow.storeOutTime" label='出库时间' show-overflow-tooltip>
              <template slot-scope="scope">{{scope.row.storeOutTime|time('y-m-d')}}</template>
            </el-table-column>
						...
						<el-table-column v-if="tableShow.boxNum" label='数量' prop='boxNum' show-overflow-tooltip></el-table-column>
						...
						<el-table-column label='理货' width="50px">
							<template slot-scope='scope'>
                <i class="iconfont yiwancheng1 icon_finish"></i>
              </template>
						</el-table-column>
            <el-table-column label="出库" width="50px">
              <template slot-scope="scope">
					...
					<ul class="card_ul">
						<li class="card_li" v-for='(val,key) in dataList' :key='key'>
							<div class="card_li_con">
								<div class="content" @click="$router.push({name:'outDeta',query:{id:val.id}})">
									<div class="content_top">
										<span class="content_top_round" :class="{ac:val.state==9}"></span>
    ...
    computed:{
      ...
      tableShow:function() {
        ...
        return temp;
			...
			selections:function(){
				return JSON.parse(this.$store.state.setJson).selection.OL;
			},
			tableOptions:function(){
				return JSON.parse(this.$store.state.setJson).tableOptions.OL;
			}
    },
		methods: {
      // 获取数据
