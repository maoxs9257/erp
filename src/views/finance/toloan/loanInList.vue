<template lang="html">
	<div class="listModule">
		<div class="list_header">
			<ym-hbt :to='{name:"toloanHome"}' title='借入列表'></ym-hbt>
		</div>
		<div class="list_setbar">
			<div class="list_setbar_options">
				...
				<div class="option" v-if="multipleSelection.length===0" @click='newLoanIn'>
        ...
        <div class="selection" v-if='selections[1].show' :class="{now:option[selections[1].key]}">
          <el-select v-model="option[selections[1].key]" :placeholder="selections[1].name" @change='screen' clearable>
            <el-option v-if='v&&v!==""' v-for='(v,k) in options[selections[1].key]' :label="toAccountName[v]" :value="v" :key='k'></el-option>
          </el-select>
        </div>
		...
		<div class="list_main" :class="{se:isSelect}">
			<div class="list_main_table" v-if='isTable'>
					...
					<el-table :data='dataList' height='100%' @selection-change="handleSelectionChange" v-loadMethod='load' modname='.el-table__body-wrapper' loadmore='loadmore'>
						...
						<el-table-column label='流水号' prop='financeSn' show-overflow-tooltip>
							<template slot-scope='scope'>
								<a href="javascript:void(0)" @click='openLoanIn(scope.row)'>{{scope.row.financeSn}}</a>
							</template>
						</el-table-column>
              ...
              <template slot-scope="scope">{{scope.row.createTime|time('y-m-d')}}</template>
						...
						<el-table-column v-if="tableShow.fromAccountName" label='债权人' prop='fromAccountName' show-overflow-tooltip></el-table-column>
						<el-table-column v-if="tableShow.toAccountName" label='资金账户' prop='toAccountName' show-overflow-tooltip></el-table-column>
						<el-table-column v-if="tableShow.money" label='借入金额' prop='money' show-overflow-tooltip></el-table-column>
            <el-table-column v-if="tableShow.createName" label="创建人" prop="createName"></el-table-column>
						<el-table-column label='结清' align="center">
							<template slot-scope='scope'>
                <i class="iconfont weijinhang icon_noFinish" v-if="scope.row.state!=2"></i>
                <i class="iconfont yiwancheng1 icon_finish" v-if="scope.row.state==2"></i>
							</template>
						</el-table-column>
					</el-table>
				</div>
					...
					<ul class="card_ul">
						<li class="card_li" v-for='(val,key) in dataList' :key='key'>
							<div class="card_li_con">
								<div class="content" @click="openLoanIn(val)">
									<div class="content_top">
										<span class="content_top_round" :class="{ac:val.state==2}"></span>
    ...
    <el-dialog title='设置' center :visible.sync='setVisible' width='800px' v-dialogDrag>
      <div>
		...
		<!-- 新建借入 -->
		<ym-loi ref='loin' @re='refresh'></ym-loi>
		<!-- 借入资金 -->
		<el-dialog width="580px" v-dialogDrag title="借入资金" :visible.sync="dialog1" center>
			<div class='newIn'>
				<ul>
					<li>
						<span>债权人：</span><span>{{loanDeta.fromAccountName}}</span>
					</li>
					<li>
					...
					<textarea cols="30" rows="10" placeholder="备注" style="resize:none"></textarea>
					<div style='margin-top:30px;text-align:center'>
						<el-button type='primary' v-if="loanDeta.borrowingPrice>0" @click="openPayMoney(loanDeta)">归还</el-button>
		...
		<!-- 归还借款弹窗 -->
		<el-dialog width="500px" v-dialogDrag title="归还借款" :visible.sync="dialog2" center>
			<div class='newIn'>
				<ul>
					<li>
						<i class="iconfont icon-shoukuan"></i>
						<input class="howMoney" type="text" v-model="barkMoney.borrowingPrice">
					</li>
					<li>
		...
		<!-- 资金明细 -->
		<el-dialog width="740px" :title='"债权人："+loanDeta.fromAccountName' center :visible.sync='flowVisible' v-dialogDrag>
      <el-table :data='flowData' height='340px'>
        <el-table-column align='center' label='发生时间' width='160'>
          <template slot-scope='scope'>{{scope.row.createTime|time('y-m-d')}}</template>
        </el-table-column>
        <el-table-column align='center' label='资金账户' prop='fromAccountName'></el-table-column>
        <el-table-column align='center' label='流水类型'>
          <template slot-scope='scope'>{{scope.row.type==3?'资金借入':'还钱'}}</template>
        </el-table-column>
        <el-table-column align='center' label='发生金额'>
...
<script>
	...
	import In from '@/components/loanin.vue'
	...
	export default {
	  ...
	  computed:{
		  ...
		  tableShow:function() {
			...
			selections:function(){
				return JSON.parse(this.$store.state.setJson).selection.LL;
			},
			tableOptions:function(){
				return JSON.parse(this.$store.state.setJson).tableOptions.LL;
			}
	  },
		methods: {
			// 获取数据
      ...
      getOptions(options){
        ...
        .then(res=>{
            ...
            let createName = {};
        		let toAccountName = {};
            data.forEach((val)=>{
            	temp.fromAccountName.push(val.fromAccountName);
            	temp.toPaymentAccountId.push(val.toPaymentAccountId);
            	temp.creatorId.push(val.creatorId);
				      createName[val.creatorId] = val.createName;
				      toAccountName[val.toPaymentAccountId]=val.toAccountName;
            });
            temp.fromAccountName = this.uniq(temp.fromAccountName);
            temp.toPaymentAccountId = this.uniq(temp.toPaymentAccountId);
      ...
      exportExcel(){
        ...
        require.ensure([],()=>{
          var tHeader=['流水号','发生时间','债权人','资金账户','发生金额','创建人'];
          var filterVal=['financeSn','createTime','fromAccountName','toAccountName','money','createName'];
