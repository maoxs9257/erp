<template lang="html">
	<div class="listModule">
		<div class="list_header">
			<ym-hbt :to='{name:"repayHome"}' title='清关公司应付'></ym-hbt>
		</div>
        ...
        <div class="option" v-if='multipleSelection.length!==0' @click="exportExcel">
          ...
          <input type="text" class="search_input_placeholder" v-model="option.companyName" @change="screenBar" placeholder="清关公司/联系人/手机号">
				</div>
			</div>
							...
							<div class="card_li_con">
								<div class="content" @click="openClear(val.id)">
									...
									<div class="content_right">
										...
										<div class="content_right_row row2">
											<span>应付</span>
											<span class="v">{{val.shouldCost}}</span>
										</div>
									</div>
								</div>
							</div>
		...
		<!-- 详情 -->
    <el-dialog width="800px" title="清关公司" center :visible.sync="visible" v-dialogDrag>
        ...
        <el-tab-pane label="路线设置" name="3">
          <div class="main">
            <div class="main_set">
              <el-table :data="lineData" height="100%">
                ...
                <el-table-column label='价钱'>
                  <template slot-scope='scope'>
                    <el-input-number size="mini" v-model="scope.row.price" :min="1"></el-input-number>
                  </template>
                </el-table-column>
              </el-table>
            </div>
            <div class="main_add">
              ...
              <el-input-number size="mini" v-model="linePrice" :min="0"></el-input-number>
    ...
    computed:{
      ...
      tableShow:function() {
        ...
        return temp;
			...
			selections:function(){
				return JSON.parse(this.$store.state.setJson).selection.RCL;
			},
			tableOptions:function(){
				return JSON.parse(this.$store.state.setJson).tableOptions.RCL;
			}
    },
		methods: {
      /**
      ...
      */
      modifyClear(){
        const loading = this.$loading({lock: true, text: '修改清关公司信息', spinner: 'el-icon-loading', background: 'rgba(0, 0, 0, 0.7)'});
      ...
      */
      getClearList(){
        ...
        }),()=>{loading.close();this.$message.error('获取清关信息失败')})
      },
      /**
      * 获取清关单
      * @method getClear
      */
      getClear(){
        return this.$axios.get('/apt/purposeForm/showPurposeFormList?tenantId='+this.tenantId+'&clearanceId='+this.infoData.id)
      },
      /**
      * 获取路线
      * @method getLine
      */
      getLine(){
        let params = {contactId: this.infoData.id};
        return this.$axios.get('/aps/customerCompany/showCompanyLine',{params});
      },
      //打开截图框
      openCropper(){
        let option = {
