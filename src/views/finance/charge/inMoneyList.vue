<template>
	<div class="listModule">
		<div class="list_header">
			<v-head :to='{name:"chargeHome"}' title="收入列表"></v-head>
		</div>
		<div class="list_setbar">
			<div class="list_setbar_options">
				...
				<div class="option" @click="openinmoney">
					...
					<el-table height="100%" :data='tableData' v-loadMethod='load' modname='.el-table__body-wrapper' loadmore='loadmore'>
						...
						<el-table-column label='流水号' show-overflow-tooltip>
							<template slot-scope='scope'>
								<a href="javascript:void(0)" @click="openModifyIn(scope.row)">{{scope.row.financeSn}}</a>
							</template>
						</el-table-column>
						...
						<el-table-column v-if='tableShow.type' label='收支类别' prop='expenseName' show-overflow-tooltip>
					...
					<ul class="card_ul">
						<li class="card_li" v-for='(val,key) in tableData' :key='key'>
							<div class="card_li_con">
								<div class="content" @click="openModifyIn(val)">
		...
		<!-- 计收入 -->
		<v-inmoney ref='inmoney' @re="refresh"></v-inmoney>
		<!-- 设置 -->
		...
		<!-- 修改 -->
		<el-dialog width="580px" title="修改收入" center :visible.sync="modifyVisible" v-dialogDrag>
			...
			<div style="text-align:center;margin-top:30px;">
				...
				<el-button type="primary" @click="modifyIn">确定</el-button>
			</div>
		</el-dialog>
	</div>
</template>
...
<script>
	...
	import inmaney from "@/components/inMoney.vue"
	...
	export default {
		...
		computed:{
			...
			tableOptions:function(){
				return JSON.parse(this.$store.state.setJson).tableOptions.IM;
			}
		},
		methods: {
			/**
			...
			*/
			...
			// 修改收入
			modifyIn(){
				let loading = this.$loading({lock: true, text: '修改收入', spinner: 'el-icon-loading', background: 'rgba(0, 0, 0, 0.7)'});
				let data = {
